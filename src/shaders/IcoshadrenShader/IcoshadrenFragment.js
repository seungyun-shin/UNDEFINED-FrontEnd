...
float cnoise(vec4 P, vec4 rep){
  ...
  return 2.2 * n_xyzw;
    ...
    void main(){
        ...
        float fresnel = pow( .7 + dot(eyeVector, normal), 2.);
        ...
        gl_FragColor = t * (1. - fresnel);
                                                    ...
                                                    // gl_FragColor = vec4(abs(sin(t * (1. - fresnel) * diff*10.)));

        
                                                    // gl_FragColor = vec4(abs(sin(diff*10.)));
                                                    // vec4 txt = texture2D(landscape, vUv);

        
        float fresnel2 = abs(dot(cameraPosition, vNormal));
        fresnel2 = fresnel2*fresnel2*fresnel2;
        // fresnel2 = fresnel2*fresnel2*0.7;
        ...
        // gl_FragColor = vec4( mix(vec3(1.), txt.rgb, fresnel2), 1.);
        // gl_FragColor = vec4(fakeUV, 0.0, 1.);
        gl_FragColor = txt;
        // gl_FragColor = vec4(fresnel2);
    }
`
export default IcoshadrenFragment;